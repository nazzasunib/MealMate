...
var ACTION_PERMS = {
  ...
  "delete-member": "members.remove", "delete-member-keep": "members.remove", "delete-member-all": "members.remove", "confirm-delete-member-all": "members.remove",
...
var WRITE_ACTIONS = {
  "toggle-member-status": 1, "delete-member-keep": 1, "confirm-delete-member-all": 1, "set-meal-default": 1, "set-meal": 1, "set-meal-all": 1, "confirm-delete": 1,
...
/* Members that count in a month's meals / hisab: everyone active, plus
   members removed "with their data kept" if they were still around at some
   point that month (mealFor() stops counting them after leftAt). */
function membersInMonth(monthKey) {
  var monthStart = monthKey + "-01";
  return DB.members.filter(function (m) {
    return m.status === "ACTIVE" || (m.status === "REMOVED" && m.leftAt && m.leftAt >= monthStart);
  });
}
...
function computeMonthSummary(monthKey) {
  ...
  var members = membersInMonth(monthKey);
  var totalMeals = 0;
...
function renderMembers() {
  var roster = DB.members.filter(function (m) { return m.status !== "REMOVED"; });
  var canDelete = can("members.remove");
    ...
    return (
      '<tr class="border-b border-border transition-colors last:border-0 hover:bg-gray-100/50">' +
        ...
        '<td class="px-5 py-3"><div class="flex items-center justify-end gap-1">' +
          ...
          (canDelete && !(m.userId && CTX && m.userId === CTX.userId) ? '<button data-action="delete-member" data-id="' + m.id + '" title="Delete member" class="rounded-lg p-2 text-muted transition-colors hover:bg-danger-100 hover:text-danger-600">' + icon("trash", "h-4 w-4") + "</button>" : "") +
        "</div></td>" +
      "</tr>"
    );
  }).join("");
  ...
  return (
    '<div class="space-y-6">' +
      ...
      '<div class="grid grid-cols-2 gap-4 sm:max-w-xs">' +
        ...
        statCard({ label: "Total Members", value: roster.length, icon: "users", tone: "gray" }) +
      "</div>" +
    ...
    modalWrapper("delete-member-modal", "Delete member?",
      '<p id="delete-member-text" class="text-sm text-muted"></p>' +
      '<div class="mt-4 space-y-2">' +
        '<button type="button" data-action="delete-member-keep" class="w-full rounded-xl border border-border p-4 text-left transition-colors hover:border-navy-600 hover:bg-navy-50/50">' +
          '<span class="block text-sm font-semibold text-foreground">Remove, keep their data</span>' +
          '<span class="mt-0.5 block text-xs text-muted">Their meals and deposits up to today stay in the hisab. Nothing new is added for them from tomorrow.</span>' +
        "</button>" +
        '<button type="button" data-action="delete-member-all" class="w-full rounded-xl border border-border p-4 text-left transition-colors hover:border-danger-600 hover:bg-danger-100/50">' +
          '<span class="block text-sm font-semibold text-danger-600">Delete with all their data</span>' +
          '<span class="mt-0.5 block text-xs text-muted">Removes every meal and deposit of theirs from all months. The hisab is recalculated without them.</span>' +
        "</button>" +
      "</div>" +
      '<div class="mt-5 flex justify-end"><button type="button" data-action="close-modal" data-modal="delete-member-modal" class="btn-secondary">Cancel</button></div>') +
    modalWrapper("delete-member-all-modal", "Are you sure?",
      '<p id="delete-member-all-text" class="text-sm text-muted"></p>' +
      '<div class="mt-5 flex justify-end gap-2"><button type="button" data-action="close-modal" data-modal="delete-member-all-modal" class="btn-secondary">No</button><button type="button" data-action="confirm-delete-member-all" class="btn-danger">Yes, delete everything</button></div>')
  );
}
...
function mealFor(memberId, dateKey) {
  ...
  /* Removed (data kept): nothing counts after their last day. */
  if (member && member.status === "REMOVED" && member.leftAt && dateKey > member.leftAt) return NO_MEALS;
...
var PENDING_MEMBER_DELETE = null;
...
on(document, "click", function (e) {
  ...
  if (action === "export-backup") {
    ...
    var delMem = memberById(el.dataset.id);
    if (delMem) {
      PENDING_MEMBER_DELETE = delMem.id;
      document.getElementById("delete-member-text").textContent = "What should happen to " + delMem.name + "'s meals and deposits?" + (delMem.userId ? " Their account will also lose access to this MealMate." : "");
      document.getElementById("delete-member-modal").hidden = false;
    }
  } else if (action === "delete-member-all") {
    var allMem = memberById(PENDING_MEMBER_DELETE);
    if (allMem) {
      document.getElementById("delete-member-modal").hidden = true;
      document.getElementById("delete-member-all-text").textContent = "This permanently deletes " + allMem.name + " and all of their meals and deposits. It can't be undone.";
      document.getElementById("delete-member-all-modal").hidden = false;
    }
  } else if (action === "delete-member-keep" || action === "confirm-delete-member-all") {
    var keepData = action === "delete-member-keep";
    var targetId = PENDING_MEMBER_DELETE;
    var delTarget = memberById(targetId);
    PENDING_MEMBER_DELETE = null;
    document.getElementById(keepData ? "delete-member-modal" : "delete-member-all-modal").hidden = true;
    if (!delTarget) return;
    if (keepData) {
      CTX.api.deleteMember(targetId, true).then(function () {
        delTarget.status = "REMOVED"; delTarget.leftAt = latestDateAcross(); delTarget.userId = null;
        toast(delTarget.name + " removed. Their history is kept.", "success");
        TEAM.loaded = false;
        render();
      }).catch(function (err) { toast(CTX.friendlyError(err), "error"); });
      return;
    }
    /* Full delete: first take away the linked account's access (if it is
       still in the group), then drop the roster row and every meal /
       deposit / buyer reference locally -- the sync layer deletes exactly
       those rows on the server, so nothing of theirs is left behind. */
    var revoke = delTarget.userId
      ? CTX.api.removeAccount(delTarget.userId).catch(function (err) {
          if (String((err && err.message) || "").indexOf("NOT_A_MEMBER") < 0) throw err;
        })
      : Promise.resolve();
    revoke.then(function () {
      DB.members = DB.members.filter(function (m) { return m.id !== targetId; });
      DB.meals = DB.meals.filter(function (m) { return m.memberId !== targetId; });
      DB.deposits = DB.deposits.filter(function (d) { return d.memberId !== targetId; });
      DB.expenses.forEach(function (x) {
        if (x.buyerIds && x.buyerIds.indexOf(targetId) >= 0) x.buyerIds = x.buyerIds.filter(function (b) { return b !== targetId; });
      });
      saveDB();
      toast(delTarget.name + " and all their data deleted.", "success");
      TEAM.loaded = false;
      render();
    }).catch(function (err) { toast(CTX.friendlyError(err), "error"); });
