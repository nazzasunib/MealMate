...
var ACTION_PERMS = {
  "add-member": "members.manage", "edit-member": "members.manage", "toggle-member-status": "members.manage", "set-meal-default": "members.manage",
  ...
  return (
    '<div class="space-y-6">' +
      ...
      '<div class="flex flex-wrap gap-3">' +
        ...
        (can("members.manage") ? quickActionLink("user-plus", "Add Member", "members") : "") +
        (can("invites.manage") ? quickActionLink("shield-check", "Invite Members", "team") : "") +
      "</div>" +
      (can("invites.manage") ? dashboardInviteCardHtml() : "") +
...
function renderMembers() {
  ...
  return (
    '<div class="space-y-6">' +
      '<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">' +
        '<div><h1 class="text-2xl font-semibold tracking-tight text-foreground">Members</h1><p class="mt-1 text-sm text-muted">Only active members are available when recording meals or expenses.' + (can("members.view") ? ' Login accounts, roles and invites live in <a href="#/team" class="font-semibold text-navy-900 hover:underline">Team &amp; Invites</a>.' : "") + "</p></div>" +
        '<button data-action="add-member" class="btn-primary">' + icon("user-plus", "h-4 w-4") + "Add Member</button>" +
      "</div>" +
      ...
      "</div>" +
    "</div>" +
    memberFormModal(null) + memberFormModal("__EDIT__")
  );
}
...
function mealFor(memberId, dateKey) {
  ...
  if (row) return row;
  return memberDefaultMeals(memberById(memberId));
}
...
function parseExpenseItemRows(form, monthKey) {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    if (!itemName && !amountRaw) continue; /* fully empty row: silently skip */
...
var TEAM = { loaded: false, loading: false, accounts: [], invite: null, error: "" };
...
on(document, "click", function (e) {
  ...
  if (action === "export-backup") {
    ...
    var addModal = document.getElementById("member-add-modal");
    var f = addModal.querySelector("form");
    if (f) f.reset();
    setPicturePreview("member-add-picture-preview", null, "?", "#888");
    if (f) setDefaultMealsInForm(f, DEFAULT_MEAL_DEFAULTS);
...
on(document, "submit", function (e) {
  ...
  if (type === "rename-group") {
    ...
    if (mode === "edit") {
      ...
      var color = AVATAR_COLORS[DB.members.length % AVATAR_COLORS.length];
      DB.members.push({ id: uid("mem"), name: name, avatarColor: color, status: "ACTIVE", joinedAt: latestDateAcross(), phone: phone || null, email: memberEmail || null, pictureUrl: pictureUrl || null, defaultMeals: defaultMeals });
    }
    saveDB();
    document.getElementById(mode === "edit" ? "member-edit-modal" : "member-add-modal").hidden = true;
    toast(mode === "edit" ? "Member updated." : "Member added.", "success");
    render();
