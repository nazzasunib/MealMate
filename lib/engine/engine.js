...
on(document, "click", function (e) {
  ...
  if (action === "export-backup") {
    ...
    if (!delTarget) return;
    CTX.api.deleteMember(targetId, keepData).then(function () {
      if (keepData) { delTarget.status = "REMOVED"; delTarget.leftAt = latestDateAcross(); delTarget.userId = null; }
      else {
        ...
        DB.expenses.forEach(function (x) { if (x.buyerIds) x.buyerIds = x.buyerIds.filter(function (b) { return b !== targetId; }); });
      }
      toast(delTarget.name + (keepData ? " removed. Their history is kept." : " and all their data deleted."), "success");
      TEAM.loaded = false;
      render();
    }).catch(function (err) { toast(CTX.friendlyError(err), "error"); });
