...
var ACTION_PERMS = {
  ...
  "add-guest-meal": "meals.edit", "edit-guest-meal": "meals.edit", "delete-guest-meal": "meals.edit",
  ...
  "regenerate-invite": "invites.manage", "confirm-regenerate-invite": "invites.manage", "toggle-invite": "invites.manage",
...
function icon(name, cls) {
  var svg = ICONS[name] || "";
  /* Some raw SVGs also carry layout classes from their original spot
     (e.g. lock/mail/search: "absolute left-3.5 top-1/2 ...", arrow-right /
     chevron-down: "hidden sm:block"), which knocked the icon out of the row
     it's reused in. So a caller's `cls` replaces everything but the lucide
     class names. */
  if (cls) {
    svg = svg.replace(/class="([^"]*)"/, function (_, orig) {
      var keep = orig.split(/\s+/).filter(function (c) { return /^lucide\b/.test(c); });
      return 'class="' + keep.concat(cls).join(" ") + '"';
    });
  }
  return svg;
}
...
/* ctx: {
     ...
     api: { signOut, updateProfile, changePassword, listAccounts, getInvite,
            regenerateInvite, setInviteEnabled, changeRole, removeAccount,
  ...
  return (
    '<div class="flex items-center gap-2">' +
      '<div class="flex items-center gap-1 rounded-xl border border-border bg-surface p-1 shadow-[var(--shadow-card)]">' +
        ...
        '<span class="min-w-[11rem] whitespace-nowrap text-center text-sm font-semibold text-foreground">' + dateLabelLong(dateKey) + "</span>" +
      ...
      "</div>" +
      /* Calendar picker: the button opens the (visually hidden) native date
         input, whose "change" jumps straight to the picked day. */
      '<div class="relative">' +
        '<button type="button" data-action="open-date-picker" title="Pick a date" class="flex h-[2.375rem] w-[2.375rem] items-center justify-center rounded-xl border border-border bg-surface text-navy-900 shadow-[var(--shadow-card)] transition-colors hover:bg-navy-50">' + icon("calendar-days", "h-4 w-4") + "</button>" +
        '<input type="date" data-action="date-pick" value="' + esc(dateKey) + '" tabindex="-1" aria-hidden="true" class="pointer-events-none absolute bottom-0 left-0 h-px w-px opacity-0" />' +
      "</div>" +
...
function computeMonthSummary(monthKey) {
  ...
  dates.forEach(function (d) {
    members.forEach(function (m) {
      totalMeals += totalMealUnitsFor(m.id, d);
    });
  });
...
/* Guest meals: extra one-off meals a member hosts for guests on a given
   day. These ADD to the host's own meal count everywhere totals are
   shown/used (Dashboard, Meals page, Settlement, Summary). */
function guestMealsFor(memberId, dateKey) {
  return DB.guestMeals.filter(function (g) { return g.hostMemberId === memberId && g.date === dateKey; });
}
function guestMealUnitsFor(memberId, dateKey) {
  return guestMealsFor(memberId, dateKey).reduce(
    function (acc, g) {
      var q = g.quantity || 1;
      if (g.breakfast) acc.breakfast += q;
      if (g.lunch) acc.lunch += q;
      if (g.dinner) acc.dinner += q;
      return acc;
    },
    { breakfast: 0, lunch: 0, dinner: 0 }
  );
}
/* Regular meal (0/1 per slot) + guest units (0..N per slot) for a member/day. */
function mealUnitsFor(memberId, dateKey) {
  var meal = mealFor(memberId, dateKey);
  var guest = guestMealUnitsFor(memberId, dateKey);
  return {
    breakfast: (meal.breakfast ? 1 : 0) + guest.breakfast,
    lunch: (meal.lunch ? 1 : 0) + guest.lunch,
    dinner: (meal.dinner ? 1 : 0) + guest.dinner,
  };
}
function totalMealUnitsFor(memberId, dateKey) {
  var u = mealUnitsFor(memberId, dateKey);
  return u.breakfast + u.lunch + u.dinner;
}

function renderMeals(dateKey) {
  var rows = activeMembers().map(function (m) { return { member: m, meal: mealFor(m.id, dateKey), units: mealUnitsFor(m.id, dateKey) }; });
  var totals = rows.reduce(function (acc, r) { acc.b += r.units.breakfast; acc.l += r.units.lunch; acc.d += r.units.dinner; return acc; }, { b: 0, l: 0, d: 0 });
  ...
  var tableRows = rows.map(function (r) {
    var total = r.units.breakfast + r.units.lunch + r.units.dinner;
    return (
      '<tr class="border-b border-border transition-colors last:border-0 hover:bg-gray-100/50">' +
  ...
  var memberMonthTotals = members.map(function (m) {
    var total = monthDates.reduce(function (sum, d) {
      return sum + totalMealUnitsFor(m.id, d);
    }, 0);
    return { member: m, total: total };
  });
  var monthGrandTotal = memberMonthTotals.reduce(function (sum, r) { return sum + r.total; }, 0);
  /* Latest date first, paged by day-of-month: 21st-end, 11th-20th, 1st-10th
     (so a 31-day month's first page holds 11 rows). */
  var summaryPages = [[], [], []];
  monthDates.slice().reverse().forEach(function (d) {
    var day = Number(d.slice(8, 10));
    summaryPages[day > 20 ? 0 : day > 10 ? 1 : 2].push(d);
  });
  summaryPages = summaryPages.filter(function (p) { return p.length > 0; });
  if (MEAL_SUMMARY_PAGE.monthKey !== monthKey) MEAL_SUMMARY_PAGE = { monthKey: monthKey, page: 0 };
  var summaryPage = Math.min(MEAL_SUMMARY_PAGE.page, Math.max(summaryPages.length - 1, 0));
  var summaryPager = summaryPages.length <= 1 ? "" :
    '<div class="flex items-center justify-between gap-3 border-t border-border px-5 py-3">' +
      '<p class="text-xs text-muted">Page ' + (summaryPage + 1) + " of " + summaryPages.length + "</p>" +
      '<div class="flex items-center gap-1">' +
        '<button data-action="meal-summary-page" data-page="' + (summaryPage - 1) + '"' + (summaryPage === 0 ? " disabled" : "") + ' class="rounded-lg p-1.5 text-muted transition-colors hover:bg-gray-100 hover:text-foreground disabled:pointer-events-none disabled:opacity-40">' + icon("chevron-left", "h-4 w-4") + "</button>" +
        summaryPages.map(function (_, i) {
          return '<button data-action="meal-summary-page" data-page="' + i + '" class="h-8 min-w-[2rem] rounded-lg px-2 text-xs font-semibold transition-colors ' + (i === summaryPage ? "bg-navy-900 text-white" : "text-muted hover:bg-gray-100 hover:text-foreground") + '">' + (i + 1) + "</button>";
        }).join("") +
        '<button data-action="meal-summary-page" data-page="' + (summaryPage + 1) + '"' + (summaryPage === summaryPages.length - 1 ? " disabled" : "") + ' class="rounded-lg p-1.5 text-muted transition-colors hover:bg-gray-100 hover:text-foreground disabled:pointer-events-none disabled:opacity-40">' + icon("chevron-right", "h-4 w-4") + "</button>" +
      "</div>" +
    "</div>";
  var monthRows = (summaryPages[summaryPage] || []).map(function (d) {
    var dayTotal = 0;
    var cells = members.map(function (m) {
      var t = totalMealUnitsFor(m.id, d);
      dayTotal += t;
  ...
  return (
    '<div class="space-y-6">' +
      '<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">' +
        ...
        '<div class="flex flex-wrap items-center gap-2">' + dateSwitcherHtml(dateKey) + '<button data-action="add-guest-meal" data-date="' + dateKey + '" class="btn-primary">' + icon("user-plus", "h-4 w-4") + "Guest Meal</button></div>" +
      "</div>" +
      ...
      '<div class="animate-slide-up overflow-hidden rounded-2xl border border-border bg-surface shadow-[var(--shadow-card)]">' +
        ...
        '<div class="overflow-x-auto"><table class="w-full text-sm"><thead><tr class="border-b border-border bg-gray-100/60 text-left text-xs font-semibold uppercase tracking-wide text-muted"><th class="px-5 py-2">Date</th>' +
          ...
          '<th class="px-5 py-2 text-right">Total</th><th class="px-5 py-2 text-right">Actions</th></tr></thead><tbody>' + monthRows + "</tbody></table></div>" + summaryPager) +
      "</div>" +
      guestMealsListHtml(dateKey) +
    "</div>" +
    guestMealFormModal(null) + guestMealFormModal("__EDIT__") +
    deleteConfirmModal("delete-guest-meal-modal", "Delete this guest meal?", "This will remove it from the host's meal count for that day.")
  );
}
...
function guestMealFormModal(mode) {
  var isEdit = mode === "__EDIT__";
  var id = isEdit ? "guest-meal-edit-modal" : "guest-meal-add-modal";
  var memberOptions = activeMembers().map(function (m) { return '<option value="' + m.id + '">' + esc(m.name) + "</option>"; }).join("");
  var body = (
    '<form data-form="guest-meal" data-mode="' + (isEdit ? "edit" : "add") + '" class="space-y-4">' +
      (isEdit ? '<input type="hidden" name="id" />' : "") +
      "<div>" + fieldLabel("Host Member") + '<select name="hostMemberId" required class="' + FIELD_CLASS + ' cursor-pointer"><option value="" disabled selected>Select a member</option>' + memberOptions + "</select></div>" +
      "<div>" + fieldLabel("Guest Name (optional)") + '<input name="guestName" type="text" maxlength="120" placeholder="e.g. Rahim (optional)" class="' + FIELD_CLASS + '" /></div>' +
      '<div class="grid grid-cols-2 gap-4"><div>' + fieldLabel("Date") + '<input name="date" type="date" required class="' + FIELD_CLASS + '" /></div><div>' + fieldLabel("Quantity") + '<input name="quantity" type="number" min="1" step="1" value="1" required class="' + FIELD_CLASS + '" /></div></div>' +
      "<div>" + fieldLabel("Meals") +
        '<div class="flex flex-wrap gap-3">' +
          '<label class="flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm"><input type="checkbox" name="breakfast" value="true" class="h-4 w-4 rounded" />Breakfast</label>' +
          '<label class="flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm"><input type="checkbox" name="lunch" value="true" class="h-4 w-4 rounded" />Lunch</label>' +
          '<label class="flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm"><input type="checkbox" name="dinner" value="true" class="h-4 w-4 rounded" />Dinner</label>' +
        "</div>" +
      "</div>" +
      "<div>" + fieldLabel("Note (optional)") + '<input name="note" type="text" maxlength="500" placeholder="e.g. Family visit" class="' + FIELD_CLASS + '" /></div>' +
      '<div class="flex justify-end gap-2 pt-2"><button type="button" data-action="close-modal" data-modal="' + id + '" class="btn-secondary">Cancel</button><button type="submit" class="btn-primary">' + (isEdit ? "Save Changes" : "Add Guest Meal") + "</button></div>" +
    "</form>"
  );
  return modalWrapper(id, isEdit ? "Edit Guest Meal" : "Add Guest Meal", body);
}

/* Guest meal list for the Meals page — one section, filtered to the date
   currently being viewed (matches the rest of renderMeals()'s daily view). */
function guestMealsListHtml(dateKey) {
  var rows = DB.guestMeals.filter(function (g) { return g.date === dateKey; });
  if (!rows.length) return "";
  var body = rows.map(function (g) {
    var host = memberById(g.hostMemberId);
    var mealsLabel = [g.breakfast && "Breakfast", g.lunch && "Lunch", g.dinner && "Dinner"].filter(Boolean).join(", ");
    return (
      '<tr class="border-b border-border transition-colors last:border-0 hover:bg-gray-100/50">' +
        '<td class="px-5 py-3"><div class="flex items-center gap-3">' + avatarHtml(host ? host.pictureUrl : null, host ? host.name : "?", "h-8 w-8", host ? host.avatarColor : "#888") + '<span class="font-medium text-foreground">' + esc(host ? host.name : "Unknown") + "</span></div></td>" +
        '<td class="px-5 py-3 text-muted">' + (g.guestName ? esc(g.guestName) : "-") + "</td>" +
        '<td class="px-5 py-3 text-muted">' + esc(mealsLabel || "-") + "</td>" +
        '<td class="px-5 py-3 text-right font-semibold tabular-nums">' + g.quantity + "</td>" +
        '<td class="px-5 py-3"><div class="flex items-center justify-end gap-1">' +
          '<button data-action="edit-guest-meal" data-id="' + g.id + '" class="rounded-lg p-2 text-muted transition-colors hover:bg-gray-100 hover:text-foreground">' + icon("pencil", "h-4 w-4") + "</button>" +
          '<button data-action="delete-guest-meal" data-id="' + g.id + '" class="rounded-lg p-2 text-muted transition-colors hover:bg-danger-100 hover:text-danger-600">' + icon("trash", "h-4 w-4") + "</button>" +
        "</div></td>" +
      "</tr>"
    );
  }).join("");
  return (
    '<div class="animate-slide-up overflow-hidden rounded-2xl border border-border bg-surface shadow-[var(--shadow-card)]">' +
      '<div class="border-b border-border p-5"><h2 class="text-sm font-semibold text-foreground">Guest Meals -- ' + dateLabelMed(dateKey) + "</h2></div>" +
      '<div class="overflow-x-auto"><table class="w-full text-sm"><thead><tr class="border-b border-border bg-gray-100/60 text-left text-xs font-semibold uppercase tracking-wide text-muted"><th class="px-5 py-3">Host</th><th class="px-5 py-3">Guest</th><th class="px-5 py-3">Meals</th><th class="px-5 py-3 text-right">Qty</th><th class="px-5 py-3 text-right">Actions</th></tr></thead><tbody>' + body + "</tbody></table></div>" +
    "</div>"
  );
}
...
function monthSwitcherHtml(monthKey, navAction) {
  ...
  return (
    '<div class="flex items-center gap-2">' +
      ...
      "</div>" +
      '<div class="relative">' +
        '<button type="button" data-action="open-month-picker" data-nav="' + navAction + '" data-month="' + monthKey + '" title="Pick a month" class="flex h-[2.375rem] w-[2.375rem] items-center justify-center rounded-xl border border-border bg-surface text-navy-900 shadow-[var(--shadow-card)] transition-colors hover:bg-navy-50">' + icon("calendar-days", "h-4 w-4") + "</button>" +
      "</div>" +
...
/* Month picker popover opened by the calendar button above. Each month is a
   plain button carrying the switcher's own nav action, so picking one goes
   through exactly the same handler as the ‹ › arrows. */
var MONTH_SHORT = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
function monthPickerInnerHtml(year, selectedKey, navAction) {
  return (
    '<div class="mb-2 flex items-center justify-between">' +
      '<button type="button" data-action="month-picker-year" data-year="' + (year - 1) + '" class="rounded-lg p-1.5 text-muted transition-colors hover:bg-gray-100 hover:text-foreground">' + icon("chevron-left", "h-4 w-4") + "</button>" +
      '<span class="text-sm font-semibold text-foreground">' + year + "</span>" +
      '<button type="button" data-action="month-picker-year" data-year="' + (year + 1) + '" class="rounded-lg p-1.5 text-muted transition-colors hover:bg-gray-100 hover:text-foreground">' + icon("chevron-right", "h-4 w-4") + "</button>" +
    "</div>" +
    '<div class="grid grid-cols-3 gap-1">' +
      MONTH_SHORT.map(function (label, i) {
        var key = year + "-" + (i < 9 ? "0" : "") + (i + 1);
        return '<button type="button" data-action="' + navAction + '" data-month="' + key + '" class="rounded-lg px-2 py-2 text-sm font-medium transition-colors ' + (key === selectedKey ? "bg-navy-900 text-white" : "text-foreground hover:bg-navy-50") + '">' + label + "</button>";
      }).join("") +
    "</div>"
  );
}
function closeMonthPicker() {
  var pop = document.getElementById("mm-month-picker");
...
function computeHisabSummary(monthKey) {
  ...
  var memberRows = members.map(function (m) {
    ...
    var meals = 0;
    dates.forEach(function (d) {
      meals += totalMealUnitsFor(m.id, d);
    });
...
function computeMemberDailyMeals(memberId, monthKey) {
  ...
  return dates.map(function (d) {
    ...
    var total = totalMealUnitsFor(memberId, d);
...
function loadTeam() {
  ...
  Promise.all([
    can("members.view") ? CTX.api.listAccounts() : Promise.resolve([]),
    wantsInvite ? CTX.api.getInvite() : Promise.resolve(null),
    wantsRequests ? CTX.api.listJoinRequests() : Promise.resolve([]),
  ]).then(function (res) {
    TEAM.accounts = res[0] || [];
    TEAM.invite = res[1];
    TEAM.requests = res[2] || [];
    ...
    TEAM.error = CTX.friendlyError(err);
    TEAM.loaded = true;
  }).then(function () {
    TEAM.loading = false;
...
function joinRequestsSectionHtml() {
  if (!can("members.manage") || !TEAM.requests.length) return "";
...
function openGuestMealEditModal(id) {
  var g = DB.guestMeals.filter(function (x) { return x.id === id; })[0];
  if (!g) return;
  var modal = document.getElementById("guest-meal-edit-modal");
  modal.querySelector('[name="id"]').value = g.id;
  modal.querySelector('[name="hostMemberId"]').value = g.hostMemberId;
  modal.querySelector('[name="guestName"]').value = g.guestName || "";
  modal.querySelector('[name="date"]').value = g.date;
  modal.querySelector('[name="quantity"]').value = g.quantity;
  modal.querySelector('[name="breakfast"]').checked = !!g.breakfast;
  modal.querySelector('[name="lunch"]').checked = !!g.lunch;
  modal.querySelector('[name="dinner"]').checked = !!g.dinner;
  modal.querySelector('[name="note"]').value = g.note || "";
  modal.hidden = false;
}
...
/* Which page of the Meals page's Monthly Meal Summary table is showing;
   resets to the first (latest) page whenever the month changes. */
var MEAL_SUMMARY_PAGE = { monthKey: null, page: 0 };

/* ============================== EVENT DELEGATION ============================== */
/* Clicking anywhere in a date field opens its calendar, not just the icon. */
on(document, "click", function (e) {
  var t = e.target;
  if (t && t.tagName === "INPUT" && t.type === "date" && !t.disabled && !t.readOnly) {
    try { t.showPicker(); } catch (err) {}
  }
  /* a click outside the month picker popover closes it */
  if (t && t.closest && !t.closest("#mm-month-picker") && !t.closest('[data-action="open-month-picker"]')) closeMonthPicker();
});
on(document, "click", function (e) {
  ...
  if (action === "open-month-picker") {
    var wasOpenHere = el.parentNode.querySelector("#mm-month-picker");
    closeMonthPicker();
    if (wasOpenHere) return;
    var pop = document.createElement("div");
    pop.id = "mm-month-picker";
    pop.dataset.nav = el.dataset.nav;
    pop.dataset.month = el.dataset.month;
    pop.className = "absolute right-0 top-full z-40 mt-2 w-64 rounded-xl border border-border bg-surface p-3 shadow-[var(--shadow-popover)]";
    pop.innerHTML = monthPickerInnerHtml(Number(el.dataset.month.slice(0, 4)), el.dataset.month, el.dataset.nav);
    el.parentNode.appendChild(pop);
    return;
  }
  if (action === "month-picker-year") {
    var yearPop = document.getElementById("mm-month-picker");
    if (yearPop) yearPop.innerHTML = monthPickerInnerHtml(Number(el.dataset.year), yearPop.dataset.month, yearPop.dataset.nav);
    return;
  }
  closeMonthPicker();
  ...
  if (action === "export-backup") {
    ...
    var pickerInput = el.parentNode.querySelector('input[data-action="date-pick"]');
    if (pickerInput) {
      try { pickerInput.showPicker(); } catch (err) { pickerInput.focus(); pickerInput.click(); }
    }
  } else if (action === "meal-summary-page") {
    MEAL_SUMMARY_PAGE.page = Math.max(0, Number(el.dataset.page) || 0);
    render();
  ...
  } else if (action === "add-guest-meal") {
    var gModal = document.getElementById("guest-meal-add-modal");
    var gf = gModal.querySelector("form");
    if (gf) gf.reset();
    gModal.querySelector('[name="date"]').value = el.dataset.date || latestDateAcross();
    gModal.hidden = false;
  } else if (action === "edit-guest-meal") {
    openGuestMealEditModal(el.dataset.id);
  } else if (action === "delete-guest-meal") {
    PENDING_DELETE = { type: "guest-meal", id: el.dataset.id };
    document.getElementById("delete-guest-meal-modal").hidden = false;
    ...
    if (PENDING_DELETE) {
      ...
      } else if (PENDING_DELETE.type === "guest-meal") {
        DB.guestMeals = DB.guestMeals.filter(function (x) { return x.id !== PENDING_DELETE.id; });
      }
      saveDB();
...
on(document, "change", function (e) {
  ...
  if (changeAction === "date-pick") {
    if (/^\d{4}-\d{2}-\d{2}$/.test(e.target.value)) navigate("meals", { date: e.target.value });
    return;
  }
...
on(document, "submit", function (e) {
  ...
  if (type === "rename-group") {
    ...
    render();
  } else if (type === "guest-meal") {
    var gmode = form.dataset.mode;
    var hostMemberId = String(fd.get("hostMemberId") || "");
    var guestName = String(fd.get("guestName") || "").trim();
    var gdate = String(fd.get("date") || "");
    var quantity = parseInt(fd.get("quantity"), 10);
    var gbreakfast = fd.get("breakfast") === "true";
    var glunch = fd.get("lunch") === "true";
    var gdinner = fd.get("dinner") === "true";
    var gnote = String(fd.get("note") || "").trim();
    if (!hostMemberId) { toast("Please select a host member.", "error"); return; }
    if (!gdate) { toast("Please select a date.", "error"); return; }
    if (!(quantity > 0)) { toast("Please enter a valid quantity.", "error"); return; }
    if (!gbreakfast && !glunch && !gdinner) { toast("Please select at least one meal (Breakfast, Lunch or Dinner).", "error"); return; }
    if (gmode === "edit") {
      var gid = String(fd.get("id") || "");
      var g = DB.guestMeals.filter(function (x) { return x.id === gid; })[0];
      if (g) { g.hostMemberId = hostMemberId; g.guestName = guestName || null; g.date = gdate; g.quantity = quantity; g.breakfast = gbreakfast; g.lunch = glunch; g.dinner = gdinner; g.note = gnote || null; }
    } else {
      DB.guestMeals.push({ id: uid("gm"), hostMemberId: hostMemberId, guestName: guestName || null, date: gdate, quantity: quantity, breakfast: gbreakfast, lunch: glunch, dinner: gdinner, note: gnote || null });
    }
    saveDB();
    document.getElementById(gmode === "edit" ? "guest-meal-edit-modal" : "guest-meal-add-modal").hidden = true;
    toast(gmode === "edit" ? "Guest meal updated." : "Guest meal added.", "success");
    render();
