...
export const TABLES = [
  {
    ...
    key: (r) => String(r.id),
    toDb: (r) => ({
      // left_at is only sent once set, so members still sync on a database
      // that hasn't run the left_at migration yet
      ...(r.leftAt ? { left_at: r.leftAt } : {}),
      id: String(r.id),
      user_id: r.userId || null,
      name: String(r.name || "").slice(0, 80) || "Member",
      avatar_color: r.avatarColor || "#0B1F4B",
      status: r.status === "INACTIVE" || r.status === "REMOVED" ? r.status : "ACTIVE",
